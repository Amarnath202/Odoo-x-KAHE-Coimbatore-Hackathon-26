...
const FILTER_TABS = [
  ...
  { label: 'Fully Delivered', value: 'Done' },
...
export default function Sales() {
  ...
  const orders = getStore(STORES.SALES_ORDERS);

  const filtered = useMemo(() => {
    let list = [...orders].sort((a, b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date));
  ...
  const kpi = useMemo(() => ({
    ...
    totalValue: orders.filter(o => o.status !== 'Cancelled').reduce((s, o) => s + (o.total || 0), 0),
  }), [orders]);

  const handleDelete = (id) => {
    const updated = orders.filter(o => o.id !== id);
    setStore(STORES.SALES_ORDERS, updated);
    const so = orders.find(o => o.id === id);
    addAuditLog({ user: user?.name, role: user?.role, module: 'Sales', action: 'Deleted', reference: id, oldValue: so?.status, newValue: 'Deleted' });
  ...
  return (
    <PageWrapper>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Total Orders', value: kpi.total, icon: ShoppingCart, color: 'bg-primary/10 text-primary' },
          { label: 'Confirmed', value: kpi.confirmed, icon: CheckCircle, color: 'bg-accent/10 text-accent' },
          { label: 'Partially Delivered', value: kpi.partial, icon: Clock, color: 'bg-warning/10 text-warning' },
          { label: 'Total Value', value: formatINR(kpi.totalValue), icon: TrendingUp, color: 'bg-success/10 text-success' },
        ].map((k, i) => (
          <motion.div
            key={k.label}
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-0">
                    <EmptyState
                      ...
                      action={
                        !search && (
                          <Link to="/sales/create" className="btn-primary btn-sm mt-2">
                ...
                filtered.map((order, i) => (
                  ...
                  >
                    ...
                    <td><StatusBadge status={order.status === 'Partial' ? 'Partially Delivered' : order.status === 'Done' ? 'Delivered' : order.status} /></td>
                    <td className="text-text-muted text-sm">{order.lines?.length || 0} item{order.lines?.length !== 1 ? 's' : ''}</td>
                    <td className="text-right font-semibold text-text-primary">{formatINR(order.total || 0)}</td>
                    <td className="text-center" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1">
                        <Link
                          to={`/sales/${order.id}`}
                          className="btn-ghost btn-icon btn-sm tooltip"
                          title="View Details"
                        >
